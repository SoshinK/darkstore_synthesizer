...
from dsynth.scenes.darkstore_env import get_arena_data

MP_SOLUTIONS = {
    "PickToCart": solve_smooth,
}

def parse_args(args=None):
    parser = argparse.ArgumentParser()
    parser.add_argument("-e", "--env-id", type=str, default="PickToCart", help=f"Environment to run motion planning solver on. Available options are {list(MP_SOLUTIONS.keys())}")
    
    parser.add_argument("-j", "--scene_json", type=str, required=True)
    parser.add_argument("--assets_dir", type=str, default="models")
...
def _main(args, proc_id: int = 0, start_seed: int = 0) -> str:
    ...
    env = gym.make(
        ...
        mapping_file = None,
        style_ids = [style_id],
        obs_mode='sensor_data',#args.obs_mode,
    ...
    while True:
        try:
            res = solve(env, target, goal_pose, seed=seed, debug=False, vis=True if args.vis else False)
